class PostsController < ApplicationController
  ...
  def create
    ...
    if @post.save
      ...
      render json: { success: false, error: @post.errors.full_messages.to_sentence }, status: :unprocessable_content
    end
  ...
  def create_linkedin_post
    ...
    return render json: { success: false, error: "No image files uploaded" }, status: :unprocessable_content if files.blank?
    ...
    if schedule
      ...
      if post.valid?
        ...
        if response["id"].present?
          ...
          render json: { success: false, error: "Failed to post" }, status: :unprocessable_content
        end
      else
        flash[:error] = post.errors.full_messages.to_sentence
        render json: { success: false, error: post.errors.full_messages.to_sentence }, status: :unprocessable_content
  ...
  def create_twitter_post
    ...
    if media_files.blank? && !schedule
      return render json: { success: false, error: "No media files uploaded" }, status: :unprocessable_content
    end
...
def delete_twitter_post
 ...
 def attach_photos(post, files)
  ...
  # def render_error(message, data = {}, status: :unprocessable_content)
